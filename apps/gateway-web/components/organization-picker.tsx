...
export function OrganizationPicker({
  ...
  const filtered = memberships.filter((membership) =>
    membership.tenant_name.toLowerCase().includes(query.toLowerCase())
  )
  async function createOrganization() {
    ...
    setPending(true)
    try {
      const result = await gatewayFetch<{ membership: { tenant_id: string } }>(
        "/auth/tenants",
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            Choose a workspace to manage your gateway.
          </p>
        </div>
        <Button disabled={pending} onClick={createOrganization}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((membership) => (
            <Link
              ...
              href={`/${locale}/${membership.tenant_id}/projects`}
            >
              <Card className="h-full transition-colors hover:border-primary">
                ...
                <CardContent className="text-sm text-muted-foreground">
                  Plan details unavailable · Open workspace
                </CardContent>
              </Card>
            </Link>
          ))}
